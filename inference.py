...
from util.eda import get_df_train_categories_counts, add_bg_index_to
from util.ploting import plot_examples
from util.tta import get_tta_list

from train import get_trainable_model, get_model_file_name, get_model_inference
...
def get_trained_model(cfg, device, fold: int = None):
    ...
    model_path = os.path.join(
        cfg["EXPERIMENTS"]["SAVED_DIR"]["BEST_MODEL"], get_model_file_name(cfg, fold)
    )
...
def inference_one(model, test_dataloader, device, cfg):
    ...
    with torch.no_grad():
        for step, (imgs, image_infos) in enumerate(tqdm(test_dataloader)):
            ...
            # inference (512 x 512)
            outs = get_model_inference_considered_tta(
                cfg, model, torch.stack(imgs).to(device)
            )
...
def inference(test_dataloader, device, cfg):
    if not cfg["EXPERIMENTS"]["KFOLD"]["TURN_ON"]:
        ...
        return inference_kfold(models_trained, test_dataloader, device, cfg)
...
def get_submission_file_name(cfg):
    ...
    submission_file = os.path.join(
        cfg["EXPERIMENTS"]["SAVED_DIR"]["SUBMISSION"], file_name
    )
...
def create_submission(test_dataloader, device, cfg):
    ...
    sample_submission_file = os.path.join(
        cfg["EXPERIMENTS"]["SAVED_DIR"]["SUBMISSION"], "sample_submission.csv"
    )
